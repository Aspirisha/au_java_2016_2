...
public class Tracker {
    private static final Logger logger = LoggerFactory.getLogger(ServerRequestExecutor.class);
    private static final Logger userLogger = LoggerFactory.getLogger("usermsg");

    private final Thread listeningThread;
    private final ExecutorService executorService;
    private final ServerRequestExecutor requestExecutor = new RequestExecutorImpl();
    ...
    private Tracker(int port, int maxPoolSize, int corePoolSize) {
    ...
    public static void main(String[] args) {
        ...
        while (sc.hasNextLine()) {
            ...
            switch (s[0]) {
                case startCommand:
                    if (null != server) {
                        System.out.println("ServerRequestExecutor is already running");
                        continue;
                    }
                ...
                case stopCommand:
                    if (null == server) {
                        System.out.println("ServerRequestExecutor is not running");
                        continue;
                    }
                    server.stop();
                    server = null;
    ...
    private static Tracker parseArgsAndStartServer(List<String> args) {
        ...
        return new Tracker(port, maxSize, coreSize);
    }
}
